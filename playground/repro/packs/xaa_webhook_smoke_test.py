...
class XAAWebhookSmokeTestPack:
    ...
    def run(self, verbose: bool = False) -> dict:
        ...
        webhook_url = os.getenv(
            "XCHAT_WEBHOOK_PUBLIC_URL", os.getenv("PLAYGROUND_WEBHOOK_URL", "")
        )
        has_https = webhook_url.startswith("https://") if webhook_url else False
        has_no_port = (
            ":" not in urllib.parse.urlparse(webhook_url).netloc.split("@")[-1]
            if webhook_url
            else True
        )
        ...
        steps.append(
            {
                ...
                "detail": "Set"
                if consumer_secret
                else "Not set — webhook signature verification will fail",
        ...
        if not failures and not warnings:
            summary_lines.append(
                "Environment looks ready. Follow steps 4-7 to complete setup."
            )
        else:
            if failures:
                summary_lines.append(
                    f"{len(failures)} required item(s) missing — fix before proceeding."
                )
            if warnings:
                summary_lines.append(
                    f"{len(warnings)} warning(s) — review before registering webhook."
                )
