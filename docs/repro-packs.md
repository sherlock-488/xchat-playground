...
**Root cause:** After E2EE is enabled for a conversation, the legacy `/2/dm_events` endpoint no longer returns message content. The message is only accessible via the `data.payload.encoded_event` field in the Activity Stream event (observed XAA envelope).
